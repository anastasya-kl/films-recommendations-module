...
def search(data):
    ...
    for key in matchedFilms:
        matchedFilms2[key] = get_filtered_film_ids(rtypes, matchedFilms[key])
    for key in matchedFilms2:
        matchedFilms3[key] = get_films_by_years_and_ids(ryears, matchedFilms2[key])
...
# дістаємо коди у порялку спадання кількості входжень токенів
...
'''
def cluster_users(user_data):
    # Збереження оригінального порядку користувачів
    ...
    # Використання агломеративної кластеризації
    num_clusters = int(np.ceil(len(user_data) / 5))
    agg_clustering = AgglomerativeClustering(n_clusters=num_clusters)
    clusters = agg_clustering.fit_predict(X_scaled)

    # Додавання інформації про кластер до користувачів
    for i, user_id in enumerate(user_ids):
        user_data[i]['cluster'] = clusters[i]
...
def clustering_analysis(user_id, clusters_info, dbname="fluxmain.db"):
    ...
    #neighbors_saves = [save for save in neighbors_saves if save]
    #st.write(neighbors_saves)
